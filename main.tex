...
\begin{flushright}
\begin{em}
Al Professor Leporati, per i preziosi consigli e per avermi seguito con grande professionalità nella stesura di questa tesi.
\\A Paolo A. Rossi, mio correlatore,  per avermi accompagnato ed ispirato nel mio intero percorso professionale.
\\A Giulia, che sempre mi ha sostenuto e ha reso anche i periodi più stressanti dei momenti felici, trovando ogni volta il modo giusto di prendermi.
\\Ai miei genitori, per aver seguito il mio intero percorso di formazione e per il sostegno economico in questi tre anni.
\\A Lorenzo, Lollo, Michele, Pillo ed Eleonora, per aver condiviso con me ogni momento della vita universitaria, facendomi passare tre anni pieni di risate.
\\A Giulia M. e Skyze, per non aver mai esitato a darmi una mano, con cuore.
\\Al Professor Fuligni e al Professor Ubertini, per aver acceso in me la passione per l'informatica, spronandomi sempre a dare il massimo.
\\A Giaco, Nicolas, Riccardo, Silvio e Vincenzo, per essere stati un vero team e aver raggiunto tanti obiettivi insieme.
\\A tutti gli amici e parenti con cui ho scherzato, riso, viaggiato e condiviso tutti i più bei momenti della mia vita.
\\A tutte le persone così folli da pensare di cambiare il mondo: perché solo coloro abbastanza folli da pensarlo, lo cambiano davvero.
...
\\\\Ritengo il lavoro svolto con Orbit un ottimo inizio di percorso, che mi ha permesso di conoscere meglio le meccaniche dietro la finanza decentralizzata, oltre che imparare a costruire, insieme ad un team, un prodotto con questo grado di complessità, in pochi mesi di lavoro.
\\Le mie ricerche mi hanno portato a concludere che il settore ha un enorme potenziale ma è ancora estremamente giovane: non è semplice trovare materiali che spieghino in modo efficace la DeFi e i suoi prodotti, motivo per il quale è estremamente importante continuare a svolgere ricerche di questo tipo e condividerle, mettendole a disposizione di tutti per aiutare il settore a crescere e migliorare.


\section{Prospettive future}
Nel momento in cui questa tesi viene scritta, la prima versione di Orbit è stata ufficialmente lanciata ed è operativa da alcuni mesi, con un capitale investito dagli utenti (\textit{TVL}) pari a circa $\$48.000$. \\Tuttavia, verranno implementate delle nuove funzionalità nei prossimi mesi, che per ragioni di tempo non sono state ancora inserite all'interno della piattaforma.\\\\\\Di queste, le più significative sono:

\begin{itemize}
    \item integrazione di altri protocolli che utilizzano la liquidità concentrata come \textbf{QuickSwap}\cite{quickswap} e \textbf{SushiSwap}\cite{sushiswap};
    \item implementazione di altre blockchain al di fuori di \textit{Polygon};
    \item realizzazione di una sezione di \textit{Analytics} per visualizzare l'andamento delle posizioni aperte dagli utenti nel tempo;
\end{itemize}

\noindent \\Lavorare a questi tesi è stata un'esperienza che mi ha fornito una maggior consapevolezza del lavoro che svolgo: non è stata una ricerca fine a se stessa, bensì una magnifica conclusione di un percorso che mi ha arricchito personalmente e professionalmente, spronandomi ad essere ancora più curioso e a voler dare un concreto aiuto nella costruzione di questo settore.
\\\\Il mio \textit{Project Work} è stata un'occasione unica per entrare nel vivo di questo mondo: mi ha dato la possibilità di conoscere tantissime persone appassionate del Web3, imparare nuove tecnologie, linguaggi e diversi prodotti unici e rivoluzionari.\\Ora che si è concluso da qualche mese, ho intenzione di continuare a lavorare nel settore Web3, con l'obiettivo di crescere come professionista e poter dare il mio contributo nello sviluppo delle nuove tecnologie.
