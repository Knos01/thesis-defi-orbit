...
Gli applicativi software utilizzati in DeFi vengono chiamati \textit{dApps (decentralized applications)}, ovvero particolari prodotti che utilizzano la blockchain di \textbf{Ethereum}\cite{ethereum}, una delle principali criptovalute, nota per la sua flessibilità e accessibilità.\\\\
Su tale blockchain risiedono dei particolari programmi denominati \textit{Smart Contracts}\cite{smartcontracts}, i quali si occupano di garantire sicurezza, trasparenza e irreversibilità delle operazioni avvenute sulla blockchain di Ethereum.\\
...
In tal modo, la liquidità concentrata garantirà un guadagno superiori di commissioni (da ora in poi fees) con il capitale da loro messo a disposizione.
...
\\Diverse blockchain con fee a basso costo (come \textbf{Polygon}\cite{polygon}) stanno diventando sempre più popolari, spostando l'attenzione degli sviluppatori e degli utenti verso la possibilità di costruire piattaforme e strumenti veloci ed efficaci.
\\\\Orbit dunque cavalca quest'onda di innovazione e di creatività, portando la liquidità concentrata verso un nuovo livello, costruendo uno strumento efficiente e facile da utilizzare per professionisti e piccoli investitori.

\subsection{Vantaggi per gli utenti}

Automatizzare le proprie posizioni su Uniswap v3 ha un diretto impatto sui ritorni generati da queste ultime.
...
\\Inoltre, nella prima versione del protocollo, sarà Orbit ad occuparsi dei costi di gas dovuti alle transazioni.\\

\section{Modelli di gestione di liquidità}

Nello stato attuale della DeFi, vi sono concretamente due modelli ben distinti di gestione di liquidità:  \textbf{Aggregatori}\cite{aggregator} e  \textbf{Smart Vaults}(*aggiungo reference).

\subsection{Aggregatori}

Gli Aggregatori sono delle particolari piattaforme DeFi, le quali permettono ai propri utenti di effettuare transazioni attraverso diverse dApps in un unico posto. Ciò permette di risparmiare tempo e aumentare l'efficienza delle transazioni.
\\Tuttavia, protocolli come Yearn, Beefy o Idle consentono all'utente l'utilizzo di strategie singole, tipicamente con un modello "\textit{Black Box}", ovvero senza verificarne l'effettivo funzionamento interno.

\subsection{Smarts vaults}

Contrariamente agli Aggregatori, gli Smart Vaults garantiscono un'alta personalizzazione delle strategie scelte direttamente dagli utenti.
\\Il funzionamento è relativamente semplice: un utente diventa \textit{proprietario} di un particolare Smart Contract, usandolo come account per gestire i propri assets e per connettersi ad altri protocolli per allocazioni automatiche di liquidità: tale Smart Contract è difatti uno Smart Vault.
\\\\Il modello a Smart Vault è stato poco utilizzato in passato, principalmente a causa delle alte fee dovute per attivare le strategie multiple.
\\Tuttavia, con l'avvento di blockchain sempre meno costose in termini di gas, è possibile utilizzare gli Smart Vaults per integrare facilmente nuovi protocolli e fornire all'utente la possibilità di avere un totale controllo su di essi.
\\Per tali ragioni, il modello a Smart Vault è stato scelto per la realizzazion di Orbit.

\section{Differenze di Orbit dai competitor}
