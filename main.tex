...
\\\\Il primo principio afferma che i componenti che formano un sistema software debbano essere realizzati in modo tale da poter essere riutilizzati per costruire nuove applicazioni. Un buon esempio di questo principio è quello dei mattoncini Lego: gli Smart Contracts di Orbit sono dei singoli mattoncini che combinati tra loro possono costruire strutture sempre più complesse, senza la necessità di essere riscritti da zero.
\\\\Il secondo principio invece, va contro la natura stessa degli Smart Contracts, i quali per definizione non sono aggiornabili. Tuttavia, attraverso degli opportuni Design Patterns, è possibile trovare un modo per aggiornare i contratti abbattendo i costi di distribuzione sulla blockchain (approfondimento alla sezione \ref{registry}).
...
Il Diamond Pattern è uno standard recente, che ha trovato largo uso nel mondo web3 grazie ai numerosi vantaggi presenti nell'architettura.
\\Il vantaggio principale sta nello sfruttare un \textbf{singolo smart contract} (chiamato Diamond), dunque un solo indirizzo ethereum, nel quale si trovano le funzionalità alle quali possono accedere tutti i contratti presenti nell'architettura.
\\Inoltre, tale contratto non presenta la canonica limitazione di dimensione di \textbf{24KB}, rendendo lo spazio a sua disposizione \textbf{virtualmente infinito}. 
\\Non meno importante è la sua \textbf{modificabilità}: tutte le funzioni esterne presenti nel Diamond possono essere modificate, eliminate o aggiunte in un qualsiasi momento senza dover distribuire nuovamente il contratto sulla blockchain, evitando di dover utilizzare un nuovo indirizzo.

\subsection{Creazione, aggiornamento e eliminazione di funzioni}


\\Il \textbf{Diamond Pattern} possiede una forte \textbf{modularità}. \\Al suo interno vi sono funzioni "\textit{esterne}", messe a disposizione da ulteriori contratti stateless chiamati \textbf{Facets}.
...
\\Quando un Facet viene distribuito sulla blockchain, una o più tra le sue funzioni vengono aggiunte ad uno o più Diamond. Essendo un contratto stateless, non può contenere dati (spazio di \textbf{storage}) ma può leggere e scrivere su differenti Diamond. 
...
Quando una funzione esterna è chiamata su un Diamond, viene eseguita una particolare funzione detta di \textbf{fallback}. Tale funzione determina quale facet verrà chiamato, in base ai primi \textit{4 bytes di dati}, detti \textit{function selector} (similmente ad il "Opcode" del linguaggio macchina, figura \ref{fig:facet}), per poi eseguire la funzione richiesta tramite il metodo \textbf{delegatecall}.
...
https://www.overleaf.com/project/62bcc1654ca61aaea5de0593
\begin{figure}[H]
...
Il sistema che gestisce lo spazio di archiviazione (storage) di Solidity non supporta contratti proxy come il Diamond. Per tale ragione, è necessario implementare uno storage associato al Diamond, appositamente creato per interagire con i Facet. 
\\Quest'ultimi possono condividere l'accesso alle variabili di stato dello storage, usando le stesse strutture dati nelle stesse posizioni delle celle di memoria. In questo modo i Facet sono entità separate ma con una memoria condivisa, disponibili in lettura e scrittura in qualsiasi momento dall'intera architettura.
...
L'unico punto di ingresso lato utente è rappresentato dall'interazione diretta con un contratto chiamato "\textit{PositionManagerFactory}", il quale si occupa della creazione dei Position Manager.
